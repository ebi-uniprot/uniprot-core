...
Run unit tests for a specific module and its required reactor dependencies:
...
```bash
mvn -pl core-parser verify -DskipUTs=true
```
...
Run the full lifecycle without Spotless:
...
aggregated by `jacoco-aggregate-report` at:
